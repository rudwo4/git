...
<html lang="ko">
  ...
  window.onload = function(){
	  ...
	  var buyBookStart =  document.buyinf.buyBookStart.value;
	  var buyBookEnd =  document.buyinf.buyBookEnd.value;
	  ...
	  if(stockcode == ""){
		  ...
		  var url ="BookbuyOrder?stockcode="+stockcode + "&buy_cnt="+buy_cnt+"&buy_price="+buy_price+"&buyBookStart="+buyBookStart+"&buyBookEnd="+buyBookEnd+"&username="+username+"&buy_total="+buy_total.replace(/,/g, '')+"&showMoney="+showMoney.replace(/,/g, '');
		  window.open(url,"주식 매수 신청","menubar=no , width=550, height=300");
	  }
  }
	  ...
	  var sellBookStart =  document.sellinf.sellBookStart.value;
	  var sellBookEnd =  document.sellinf.sellBookEnd.value;
	  ...
	  if(stockcode == ""){
		  ...
		  var url = "BooksellOrder?stockcode="+stockcode + "&sell_cnt="+sell_cnt+"&sell_price="+sell_price+"&sell_price="+sell_price+"&sellBookStart="+sellBookStart+"&sellBookEnd="+sellBookEnd+"&username="+username+"&sell_total="+sell_total.replace(/,/g, '');
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
		...
		<div class="col-xl-4 col-12">
	       ...
	       <!-- /.box -->
			  ...
			  <div class="box-body tab-content">
				...
				<br>  
				<div class="d-block">
					<button type="button" class="btn btn-block btn-success btn-lg" onclick="orderBuy();">예약 매수 주문</button>
				</div>
				</form>
			 </div>
		...
		<div class="col-xl-4 col-12">
			...
			<div class="box">
			<div class="box-body no-padding" style="overflow : auto;" id="slimtest2">
			<div class="table table-hover" style="height :250px; overflow : auto;" id="slimtest2">
	...
	<div class="col-lg-6 col-12">
	  ...
	  <div class="box box-solid bg-black">
		...
		<div class="box-body">
			<div class="table-responsive" style="overflow: auto; height: 400px;" id="buyBooking" >
				<table class="table table-bordered no-margin">
				  ...
				  <c:forEach var="buy" items="${buyBookStock}">
				  	<tr>
				  	  <td class="text-red hover-danger">
				  	  <fmt:formatDate value="${buy.stockDate}" pattern ="yy/MM/dd"/>
				  	  ~
				  	  <fmt:formatDate value="${buy.endstockdate}" pattern ="yy/MM/dd"/>
				  	  </td>
					  <td>${buy.stockCode}</td>
					  <td>${buy.stockTheme}</td>
					  <td>${buy.stockName}</td>
					  <td><fmt:formatNumber type="number" maxFractionDigits="3" value="${buy.stockCurrent}" />원</td>
					  <td><fmt:formatNumber type="number" maxFractionDigits="3" value="${buy.totalStock}" />주</td>
					</tr>
				  </c:forEach>
				  </tbody>
				</table>
	  ...
	  <!-- /.box -->
		...
		<div class="col-lg-6 col-12">
		  ...
		  <div class="box box-solid bg-black">
			...
			<div class="box-body">
				<div class="table-responsive" style="overflow: auto; height: 400px;" id="sellBooking">
					<table class="table table-bordered table-striped no-margin">
					  ...
					  <tbody>
						<c:forEach var="sell" items="${sellBookStock}">
					  	<tr>
					  	  <td class="text-yellow hover-warning">
					  	  <fmt:formatDate value="${sell.stockDate}" pattern ="yy/MM/dd"/>
					  	  ~
					  	  <fmt:formatDate value="${sell.endstockdate}" pattern ="yy/MM/dd"/>
					  	  </td>
						  <td>${sell.stockCode}</td>
						  <td>${sell.stockTheme}</td>
						  <td>${sell.stockName}</td>
						  <td><fmt:formatNumber type="number" maxFractionDigits="3" value="${sell.stockCurrent}" />원</td>
						  <td><fmt:formatNumber type="number" maxFractionDigits="3" value="${sell.totalStock}" />주</td>
						</tr>
					 	</c:forEach>
					  </tbody>
					</table>
