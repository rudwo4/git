...
function transferMoney(){
	...
	var sum = (possible_price.replace(/,/g, ''))-(transfer_price.replace(/,/g, ''));
	
	var reg1 =/^[0-9]{0,9}$/;
	var reg2 =/^[가-힣]{0,8}$/;
	
	var result1 = reg1.test(account_pwd);
	var result2 = reg1.test(transfer_price.replace(/,/g, ''));
	...
	if(account_pwd == ""){
		...
		var url = "transferChk?transfer_price=" + transfer_price.replace(/,/g, '') + "&deposit_account=" + deposit_account + "&withdraw=" + withdraw + "&transferMessage=" + transferMessage + "&withdrawMessage=" + withdrawMessage +"&sum=" + sum;
