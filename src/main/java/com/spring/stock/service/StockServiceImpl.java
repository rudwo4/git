...
public class StockServiceImpl implements StockService {
	...
	public void search(HttpServletRequest req, Model model) throws IOException {
		...
		// 인기 검색 종목TOP5
		String url4 ="https://finance.naver.com/";
		Document doc4 = Jsoup.connect(url4).get();
		Elements element4 = doc4.select(".aside_popular table thead");
		Elements element5 = doc4.select(".aside_popular table tbody");
		  
		model.addAttribute("thead", element4.html());
		model.addAttribute("tbody",element5.html());
		}
		
		//나의 매수/매도 예약 내역 확인
		@Override
		public void buySellBook(HttpServletRequest req, Model model) {
			String userId = req.getParameter("username");	// 아직 연결 안되어있음(로그인 시 아이디 확인 후 자동으로 화면 창 나오게함)
			
			List<Stock_StockVO> buyBookStock = dao.buyBook(userId);
			List<Stock_StockVO> sellBookStock = dao.sellBook(userId);
		...
		// 회원정보보기 - 추가분 2019.08.12 ==========> 수정 2019.08.23 
		@Override 
		public void modifyView(HttpServletRequest req, Model model) {
			...
			if(passwordEncoder.matches(strPwd, c_pwd)) { 
				...
				String username = strId;
				AccountVO account = dao.accountInfo(username);
				System.out.println("테스트~~~1~~" + account);
				
				model.addAttribute("stock_limit", account.getStock_limit());
				model.addAttribute("transfer_limit", account.getTransfer_limit());
		...
		public void c_modifyView(HttpServletRequest req, Model model) {
			...
			// 5-1. 계좌 한도 수정처리 확인 
			int accountLimit = Integer.parseInt(req.getParameter("accountLimit2"));
			System.out.println(" 계좌 한도확인!!! " + accountLimit);
			
			int stockLimit = Integer.parseInt(req.getParameter("stockLimit2"));
			System.out.println(" 계좌 한도확인!!! " + stockLimit);
			
			Map<String, Object> map = new HashMap<String, Object>();
			map.put("accountLimit", accountLimit);
			map.put("stockLimit", stockLimit);
			map.put("strId", strId);
			int limitUpdate = dao.updateLimit(map);
			
			// 6. 처리 결과를 저장 
			model.addAttribute("updateCnt", updateCnt);
			model.addAttribute("limitUpdate", limitUpdate);
		}
		...
		public void c_deletePro(HttpServletRequest req, Model model, HttpServletResponse res) {
	    	...
	    	int deleteBoardCount = dao.deleteBoardMemeber(strId);
		    if(deleteBoardCount == 1) {
		    	deleteCnt = dao.deleteMember(strId);
		    }
	    	// 세션삭제
		...
		public void detailTheme(HttpServletRequest req, Model model) {
			...
			if(theme == null) {
				...
				// 종목 분석 그래프 값 가져오기 ============== 아진 추가 2019.08.25 ================
				List<ChartVO> themeChart1 = dao.themeChart1(theme);
				List<ChartVO> themeChart2 = dao.themeChart2(theme);
				model.addAttribute("themeChart1", themeChart1);
				model.addAttribute("themeChart2", themeChart2);
        ...
        public void hostForm(HttpServletRequest req, Model model) {
           ...
           if(cnt > 0) {
              ...
              List<BoardVO> dto1 = dao.board_cnt_list();
              model.addAttribute("dto", dto);   // 큰바구니 : 게시글목록 cf) 작은바구니 : 게시글 1건
		...
		public void quiryList(HttpServletRequest req, Model model) {
			...
			if(cnt > 0) {
				...
				// 게시글 목록 조회
				List<userInquiryVO> dtos = dao.getArticleList_1on1(map);
		...
		public void contentForm(HttpServletRequest req, Model model) {
			int num = Integer.parseInt(req.getParameter("num"));
			int co_num = Integer.parseInt(req.getParameter("num"));
			
			dao.addReadCnt(num);
			userInquiryVO vo = dao.getArticle(num);
			map.put("co_num", co_num);
			// 1대1 문의 답글보기
			List<userInquiryVO> dtos = dao.showComment(map);
		...
		public void userdeletePro(HttpServletRequest req, Model model) {
			...
			map.put("num", num);
			map.put("co_num", co_num);
			dao.addbo_del_reply(username);
			dao.deletecBoard(co_num);
			int deleteCnt = 0;
			System.out.println("username : " + username);
			deleteCnt = dao.deleteBoard(num);
			// 글삭제시 -1
			int boardCnt = dao.getdeletecnt(username);
		...
		// 댓글달기
		@Override
		public void insert_comment(HttpServletRequest req, Model model) {
			userInquiryVO vo = new userInquiryVO();
			int num =Integer.parseInt(req.getParameter("num"));
			int co_num = Integer.parseInt(req.getParameter("num"));
			String writer = req.getParameter("username");
			vo.setCo_num(co_num);
			String comment = req.getParameter("editor1");
			vo.setComments(comment);
			String username = req.getParameter("username");
			vo.setUsername(username);
			map.put("num", num);
			// 같은 사람이쓴 글일경우 답글이 0개일떄 +1
			int addCnt = dao.addReply(num);
			System.out.println("addCnt : " + addCnt);
			if(addCnt == 0) {
				// 댓글 추가시 +1
				dao.addbo_reply(writer);
			}
		...
		//즉시 매수 신청
		@Override
		public void buyOrder(HttpServletRequest req, Model model) {
			String username = "jkj"; //req.getParameter("김일광이 로그인 아이디 넘겨주면 넣을 곳");
			String CompanyCode = req.getParameter("stockcode");
			int buy_cnt = Integer.parseInt(req.getParameter("buy_cnt"));
			int buy_price = Integer.parseInt(req.getParameter("buy_price"));
			int buy_total = Integer.parseInt(req.getParameter("buy_total"));
			...
			map.put("buy_total", buy_total);
			
			int buyCnt = dao.buyOrder(map);
			Stock_StockVO vo = dao.asking_stock(CompanyCode);
			
			model.addAttribute("vo",vo);
			model.addAttribute("buyCnt",buyCnt);
			model.addAttribute("buy_cnt",buy_cnt);
			model.addAttribute("buy_price",buy_price);
			model.addAttribute("buy_total",buy_total);
			
			System.out.println(username + CompanyCode + buy_cnt + buy_price + buy_total);	
		}
		
		//즉시 매도 신청
		@Override
		public void sellOrder(HttpServletRequest req, Model model) {
			String username = "jkj"; //req.getParameter("김일광이 로그인 아이디 넘겨주면 넣을 곳");
			String CompanyCode = req.getParameter("stockcode");
			int sell_cnt = Integer.parseInt(req.getParameter("sell_cnt"));
			int sell_price = Integer.parseInt(req.getParameter("sell_price"));
			int sell_total = Integer.parseInt(req.getParameter("sell_total"));
			
			Map<String,Object> map = new HashMap<String,Object>();
			map.put("stockcode", CompanyCode);
			map.put("sell_cnt", sell_cnt);
			map.put("sell_price", sell_price);
			map.put("sell_total", sell_total);
			
			int sellCnt = dao.sellOrder(map);
			
			Stock_StockVO vo = dao.asking_stock(CompanyCode);
			
			model.addAttribute("vo",vo);
			model.addAttribute("sellCnt",sellCnt);
			model.addAttribute("sell_cnt",sell_cnt);
			model.addAttribute("sell_price",sell_price);
			model.addAttribute("sell_total",sell_total);
			System.out.println(username + CompanyCode + sell_cnt + sell_price + sell_total);	
		}
		

		//8.23 trading 현금 보여주기	
		@Override
		public void showMoney(HttpServletRequest req, Model model) {
			String username = req.getParameter("username");
			AccountVO money = dao.showMoney(username);
		...
		//예약 매수
		@Override
		public void BookbuyOrder(HttpServletRequest req, Model model) {
			String username = req.getParameter("username");
			String stockcode = req.getParameter("stockcode");
			int buy_cnt = Integer.parseInt(req.getParameter("buy_cnt"));
			int buy_price = Integer.parseInt(req.getParameter("buy_price"));
			
			String buyBookStart = req.getParameter("buyBookStart");
			String BookStart[] = buyBookStart.split("-");
			int Startdate = Integer.parseInt(BookStart[0]+BookStart[1]+BookStart[2]);
					
			String buyBookEnd = req.getParameter("buyBookEnd");
			String BookEnd[] = buyBookEnd.split("-");
			int Enddate = Integer.parseInt(BookEnd[0]+BookEnd[1]+BookEnd[2]);
			
			double buy_total = Double.parseDouble((req.getParameter("buy_total")));
			int showMoney = Integer.parseInt(req.getParameter("showMoney"));
			
			System.out.println("Startdate" + Startdate);
			System.out.println("Enddate" + Enddate);
			
			try {
				if(buy_total <= showMoney) {
					if(buy_total<1000000000) {
						...
						map.put("buy_cnt", buy_cnt);
						map.put("buy_price", buy_price);
						map.put("buyBookStart", buyBookStart);
						map.put("buyBookEnd", buyBookEnd);
						
						int Bookbuy = dao.BookbuyOrder(map);
						
						model.addAttribute("Bookbuy",Bookbuy);
						model.addAttribute("stockcode",stockcode);
						model.addAttribute("buy_cnt",buy_cnt);
						model.addAttribute("buy_price",buy_price);
						model.addAttribute("buyBookStart",buyBookStart);
						model.addAttribute("buyBookEnd",buyBookEnd);
						
						System.out.println("username = " + username);
						System.out.println("Bookbuy = " + Bookbuy);
						}
					}	
				}	
			}
		...
		//예약 매도
		@Override
		public void BooksellOrder(HttpServletRequest req, Model model) {
			String username = req.getParameter("username");
			String stockcode = req.getParameter("stockcode");
			int sell_cnt = Integer.parseInt(req.getParameter("sell_cnt"));
			int sell_price = Integer.parseInt(req.getParameter("sell_price"));
			
			String sellBookStart = req.getParameter("sellBookStart");
			String BookStart[] = sellBookStart.split("-");
			int sellStartdate = Integer.parseInt(BookStart[0]+BookStart[1]+BookStart[2]);
					
			String sellBookEnd = req.getParameter("sellBookEnd");
			String BookEnd[] = sellBookEnd.split("-");
			int sellEnddate = Integer.parseInt(BookEnd[0]+BookEnd[1]+BookEnd[2]);
			
			double sell_total = Double.parseDouble((req.getParameter("sell_total")));
			
			Map<String,Object> map1 = new HashMap<String,Object>();
			map1.put("username", username);
			map1.put("stockcode", stockcode);
			map1.put("sell_cnt", sell_cnt);
			
			int CanCnt = dao.CansellStock(map1);
			System.out.println("cnt : " + CanCnt);
			
			try {
				if(sell_total<1000000000)
				if(CanCnt == 1) {
					...
					map.put("sell_cnt", sell_cnt);
					map.put("sell_price", sell_price);
					map.put("sellBookStart", sellBookStart);
					map.put("sellBookEnd", sellBookEnd);
					
					int Booksell = dao.BooksellOrder(map);
					
					model.addAttribute("Booksell",Booksell);
					model.addAttribute("stockcode",stockcode);
					model.addAttribute("sell_cnt",sell_cnt);
					model.addAttribute("sell_price",sell_price);
					model.addAttribute("sellBookStart",sellBookStart);
					model.addAttribute("sellBookEnd",sellBookEnd);
					
					System.out.println("username = " + username);
					System.out.println("Booksell = " + Booksell);
					}
				}
			}
			catch(Exception e) {
		...
		// 마이페이지 - 거래내역 종합 ===미체결 테이블 =========== 아진 추가 2019.08.27 ================
		@Override
		public void stockAll(HttpServletRequest req, Model model) {
	        // 아이디 갖고오기
	        Authentication authentication = SecurityContextHolder.getContext().getAuthentication(); 
			User user = (User) authentication.getPrincipal(); 
			String strId = user.getUsername();
			
			List<Stock_allVO> stockall = dao.stockAll(strId);
			
			model.addAttribute("stockall", stockall);
			
		}

		// 마이페이지 - 거래내역종합 ============== 아진 추가 2019.08.29 ================
		@Override
		public void stock_completed(HttpServletRequest req, Model model) {
			// 아이디 갖고오기
	        Authentication authentication = SecurityContextHolder.getContext().getAuthentication(); 
			User user = (User) authentication.getPrincipal(); 
			String strId = user.getUsername();
			
			int completedCnt = dao.getCompletedCnt(strId);
			if(completedCnt > 0) {
				List<Stock_completedVO> stockCompleted = dao.stock_comleted(strId);
				model.addAttribute("stockCompleted", stockCompleted);
			}
		...
		@Override
		public void stock_board(HttpServletRequest req, Model model) throws IOException {
			// 24개 종목 랜덤 셀렉트
			List<StockstockVO> list = new ArrayList<StockstockVO>();
			list = dao.stock_board();
			...
			// 환율 정보
			// vo 낭비하기 싫어서 기존 SearchVO 사용
			SearchVO vo;
			
			// 달러
			Elements element1 = doc.select("#exchangeList .usd");
			if (element1 != null) {
				vo = new SearchVO();
				// num : 매매기준율
				vo.setNum(element1.select(".value").text());
				// title : 전일대비
				vo.setTitle(element1.select(".change").text());
				// content : 상승/하락 기준
				vo.setContent(element1.select("span.blind").text());
				
				model.addAttribute("usd", vo);
			}

			// 유로
			Elements element2 = doc.select("#exchangeList .eur");
			if (element2 != null) {
				vo = new SearchVO();
				vo.setNum(element2.select(".value").text());
				vo.setTitle(element2.select(".change").text());
				vo.setContent(element2.select("span.blind").text());
				model.addAttribute("eur", vo);
			}

			// 엔
			Elements element3 = doc.select("#exchangeList .jpy");
			if (element3 != null) {
				vo = new SearchVO();
				vo.setNum(element3.select(".value").text());
				vo.setTitle(element3.select(".change").text());
				vo.setContent(element3.select("span.blind").text());
				model.addAttribute("jpy", vo);
			}

			// 위안
			Elements element4 = doc.select("#exchangeList .cny");
			if (element4 != null) {
				vo = new SearchVO();
				vo.setNum(element4.select(".value").text());
				vo.setTitle(element4.select(".change").text());
				vo.setContent(element4.select("span.blind").text());
				model.addAttribute("cny", vo);
			}

			// 코스피 / 코스닥
			String url2 = "https://finance.naver.com/sise/sise_index.nhn?code=KOSPI";
			Document doc2 = Jsoup.connect(url2).get();

			Elements e1 = doc2.select(".subtop_sise_detail #quotient");
			Elements e2 = doc2.select(".subtop_sise_detail .table_kos_index");

			vo = new SearchVO();

			String[] str = e1.select(".fluc").text().split(" ");
			
			// num : 전일비
			vo.setNum(e1.select("#now_value").text());
			// title : 전일비
			vo.setTitle(str[0]);
			// content : 등락율
			vo.setContent(str[1].substring(0, 6));
			// url : 거래량
			vo.setUrl(e2.select("td#quant").text());
			// date : 거래대금
			vo.setDate(e2.select("td#amount").text());
			model.addAttribute("kospi", vo);

			String url3 = "https://finance.naver.com/sise/sise_index.nhn?code=KOSDAQ";
			Document doc3 = Jsoup.connect(url3).get();

			Elements e3 = doc3.select(".subtop_sise_detail #quotient");
			Elements e4 = doc3.select(".subtop_sise_detail .table_kos_index");

			vo = new SearchVO();
			vo.setNum(e3.select("#now_value").text());

			String[] str2 = e3.select(".fluc").text().split(" ");

			vo.setTitle(str2[0]);
			vo.setContent(str2[1].substring(0, 6));
			vo.setUrl(e4.select("td#quant").text());
			vo.setDate(e4.select("td#amount").text());
			model.addAttribute("kosdaq", vo);
		}
}
